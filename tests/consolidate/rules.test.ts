...
describe("consolidate rules", () => {
  ...
  it("uses the same half-life for todo and fact entries", () => {
    const now = new Date("2026-02-18T00:00:00.000Z");
    ...
    expect(forgettingScore(todo, now)).toBeCloseTo(forgettingScore(fact, now), 12);
  });
  ...
  it("todo reaches ~0.63 after 60 days (90-day half-life)", () => {
    const now = new Date("2026-02-18T00:00:00.000Z");
    ...
    expect(forgettingScore(todo, now)).toBeCloseTo(0.63, 2);
  });

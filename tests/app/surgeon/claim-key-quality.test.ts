...
describe("claim_key_quality surgeon pass", () => {
  ...
  it("auto-converges low-risk entity family aliases and records structured evidence", async () => {
    ...
    expect(observations).toContain("Entity-family convergence auto-applied 2 entry rewrites across 1 family cluster and staged 0 unresolved family proposals.");
    expect(actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          entryIds: ["macmini-policy-alias"],
          details: expect.objectContaining({
    ...
    expect(proposals).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          entryIds: ["james-editor", "james-timezone", "jim-editor", "jim-timezone", "jm-editor", "jm-timezone"],
          proposedClaimKeys: [],
          eligibleForApply: false,
        }),
      ]),
    );
    expect(observations).toContain("Entity-family convergence auto-applied 0 entry rewrites across 0 family clusters and staged 1 unresolved family proposal.");
    expect(actions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          entryIds: ["james-editor", "james-timezone", "jim-editor", "jim-timezone", "jm-editor", "jm-timezone"],

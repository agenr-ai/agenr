...
describe("startInternalEvalServer", () => {
  it("hosts the recall and before-turn internal eval routes on one local server", async () => {
    ...
    expect(server.routePaths).toEqual([
      "/internal/evals/recall/run",
      "/internal/evals/before-turn/run",
      "/internal/evals/session-start/run",
      "/internal/evals/dreaming-efficiency/run",
    ]);
  });

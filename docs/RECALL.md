...
- Attribution sweep outcome: the phase-0 sweep at run `2026-04-19T23-07-52-044Z` classifies the two before-turn rows (22 and 23) as `threshold_induced`. They still fail on baseline after this recalibration because the seeded single-entry memory pool produces an RRF-normalized relevance of `1.0`, and the default importance/recency for the fixture compose above `0.92`. A follow-up phase-2 pass is tracked in the recall-regression-resolution plan; it must choose between a further threshold bump, a pool-size guard on the high-confidence check, or a continuation-turn-specific escalation signal, and cite rows 22 and 23 before editing this constant again.

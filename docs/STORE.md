...
- `storeEntriesDetailed()` returns aggregate counts plus per-input outcomes and is what ingest and the OpenClaw tool use
- `storeEntries()` is the smaller wrapper that returns only aggregate counts
...
interface StoreEntryInput {
  ...
  claim_key_source?: "manual" | "model" | "json_retry" | "deterministic_repair" | "surgeon_metadata_rewrite" | "surgeon_family_reuse" | "surgeon_compaction";
...
- it then calls `db.supersedeEntry(oldId, newId, "update")`
...
The OpenClaw `agenr_store` tool stores one entry at a time and calls `storeEntriesDetailed()` directly without the ingest bulk-write wrapper.

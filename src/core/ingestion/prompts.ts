...
export function buildExtractionSystemPrompt(options: { wholeFile?: boolean; extractionContext?: string; currentDate?: string } = {}): string {
  ...
  return [
    ...
    "- directive: A first-class memory behavior instruction about what to suppress or proactively surface. What MEMORY SHOULD DO.",
    "",
    "## Type Selection",
    "",
    ...
    "3. Does it instruct memory to suppress or proactively surface a topic? → directive",
    "4. Does it prescribe what to DO going forward because a person, household, team, project, or system has adopted it? → decision",
    "5. Does it record something that HAPPENED once (a migration, launch, or shift)? → milestone",
    "6. Does it connect two NAMED things? → relationship",
    ...
    '- Extraction assigns "core" only for directive entries. Other entries must use "permanent" or "temporary".',
    "",
    "## Directive Fields",
    "",
    "- For directive entries, include `claim_key` using `user/memory_directive/<name>`.",
    '- For directive entries, include `directive_polarity`: "abstain" to suppress a topic, or "proactive" to surface it.',
    '- For directive entries, include optional `directive_trigger`: "session_start", "always", or "topic:<term>". Omit it only when the default is obvious.',
    "- Use `session_start` or `always` when a proactive directive should surface at session start.",
    "- Use `topic:<term>` when a proactive directive should surface during before-turn only after the user mentions that topic.",
    '- Example abstain: `{"claim_key":"user/memory_directive/do_not_mention_stan","directive_polarity":"abstain","directive_trigger":"always"}` with content "Do not mention Stan."',
    '- Example proactive session-start: `{"claim_key":"user/memory_directive/weekly_goals","directive_polarity":"proactive","directive_trigger":"session_start"}` with content "Ask about weekly goals at session start."',
    '- Example proactive topic: `{"claim_key":"user/memory_directive/stan_check_in","directive_polarity":"proactive","directive_trigger":"topic:stan"}` with content "When Stan comes up, ask whether his async standup preference still holds."',
    "",
    ...
    '{ "type": "fact|decision|preference|lesson|relationship|milestone|directive", "subject": "2-6 word topic noun phrase", "content": "specific declarative statement grounded in the concrete person, system, relationship, routine, or situation, min 20 chars", "importance": "high|standard|low", "expiry": "permanent|temporary|core", "tags": ["1-4", "lowercase", "tags"], "source_context": "one sentence, max 20 words" }',
    'Optional when explicitly present in a source tool call: { "claim_key": "entity/attribute" }',
    'Required for directive entries: { "claim_key": "user/memory_directive/<name>", "directive_polarity": "abstain|proactive", "directive_trigger": "session_start|always|topic:<term>" }',
    "",
    "## Few-Shot Examples",
    "",

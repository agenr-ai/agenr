...
import {
  buildEntryLocalLexicalTokens as buildSharedEntryLocalLexicalTokens,
  buildTrustedClaimKeySupportSeed as buildSharedTrustedClaimKeySupportSeed,
  evaluateClaimKeyCompactness as evaluateSharedClaimKeyCompactness,
  evaluateClaimKeySupport as evaluateSharedClaimKeySupport,
  ...
  type ClaimKeyCompactnessEvaluation,
...
import type { Entry, EntryType } from "../../core/types.js";
...
type MissingBackfillSupportEvaluation = ClaimKeySupportEvaluation & {
...
function buildTrustedCleanupHintSeed(entries: Entry[]): TrustedCleanupHintSeed {
  const sharedSeed = buildSharedTrustedClaimKeySupportSeed(entries);
  const claimKeyExamples = normalizeStringArray(sharedSeed.entries.map((entry) => entry.claimKey)).slice(0, MAX_CLEANUP_CLAIM_KEY_HINTS);
...
function evaluateMissingBackfillSupport(entry: Entry, targetClaimKey: string, trustedHints: TrustedCleanupHintSeed): MissingBackfillSupportEvaluation {
  const support = evaluateSharedClaimKeySupport(entry, targetClaimKey, trustedHints);
  ...
  return {
    ...
    siblingSlotResonance: evaluateSiblingSlotResonance({
      ...
      groundedSiblings: support.supportingEntryIds
        .map((entryId) => trustedHints.entries.find((trustedEntry) => trustedEntry.id === entryId))
        .filter((trustedEntry): trustedEntry is TrustedCleanupHintEntry => Boolean(trustedEntry))
        .map((trustedEntry) => ({
...
function evaluateClaimKeyCompactness(
  ...
  return evaluateSharedClaimKeyCompactness(claimKey, prior);
}
...
function normalizeGroundingTags(tags: string[]): string[] {
  return normalizeSharedGroundingTags(tags);
}

function buildEntryLocalLexicalTokens(entry: Entry): string[] {
  return buildSharedEntryLocalLexicalTokens(entry);
}

function tokenizeGroundingText(value: string | undefined): string[] {
  return tokenizeSharedGroundingText(value);
}

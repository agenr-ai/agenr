  ...
  for (const episode of episodes) {
    ...
    const existingSessionDurables =
      episode.sessionId !== null
        ...
        : [];
    const existingClaimKeyContext = options.contextLookupEnabled
      ? await deps.port.listActiveClaimKeyContext({
          project: episode.project ?? undefined,
          entityPrefixes: extractEpisodeEntityPrefixes(episode),
          limit: CLAIM_KEY_PROMPT_CONTEXT_LIMIT,
        })
      : [];
...
export async function classifyCandidates(mined: MinedCandidate[], deps: { port: DreamPort; contextLookupEnabled: boolean }): Promise<DreamCandidate[]> {
  ...
  const claimKeyContextByEntity = new Map<string, DreamClaimKeyContextDurable[]>();
  const claimKeyContextByProject = new Map<string, DreamClaimKeyContextDurable[]>();
  ...
  for (const [index, candidate] of mined.entries()) {
    ...
    if (disposition === "new" && deps.contextLookupEnabled) {
      const sibling = await findLikelyClaimKeyContextMatch(candidate, deps.port, claimKeyContextByEntity, claimKeyContextByProject);
      if (sibling) {
        disposition = "refines";
        refinesDurableId = sibling.id;
      }
    }
...
/** Selects likely claim-key entities from episode metadata and summary text. */
function extractEpisodeEntityPrefixes(episode: DreamEpisodeEvidence): string[] {
  const prefixes: string[] = [];
  const project = normalizeClaimKeySegment(episode.project ?? "");
  if (project) {
    prefixes.push(project);
  }

  for (const token of tokenizeContextText(episode.summary)) {
    if (token.length >= 3 && token.length <= 40) {
      prefixes.push(token);
    }
  }

  return Array.from(new Set(prefixes)).slice(0, 24);
}

/** Finds an active same-family durable that a near-miss extract candidate should refine. */
async function findLikelyClaimKeyContextMatch(
...
function isLikelyClaimKeyContextMatch(candidate: MinedCandidate, durable: DreamClaimKeyContextDurable): boolean {
  if (!durable.claimKey) {
    return false;
  }
...
function hasStrongTextOverlap(left: string, right: string): boolean {
  const leftTokens = tokenizeContextText(left);
  const rightTokens = tokenizeContextText(right);
  const overlap = calculateOverlap(leftTokens, rightTokens);
...
function tokenizeContextText(text: string): Set<string> {
  return new Set(
    text
      .toLowerCase()
      .split(/[^a-z0-9]+/u)

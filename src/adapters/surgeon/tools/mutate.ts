...
import { getEntry, retireEntry } from "../../db/queries.js";
...
export function createRetireEntryTool(deps: SurgeonToolDeps): AgentTool<typeof RETIRE_ENTRY_SCHEMA> {
  return {
    ...
    async execute(_toolCallId, params: RetireEntryParams) {
      const entry = await getEntry(deps.executor, params.entry_id);
      if (!entry) {
        return toolResult({
          success: false,
          dryRun: !deps.apply,
      ...
      const retired = await retireEntry(deps.executor, entry.id, params.reason.trim());
      return toolResult({
        success: retired,
        dryRun: false,
        entryId: entry.id,
